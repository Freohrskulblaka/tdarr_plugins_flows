...
9. Confirm the Tdarr log shows `media-optimizer-hdr-static-validation-2026-09-17-54` and the expected planned track table before allowing live processing.
...
Compatibility profiles use the best eligible main soundtrack within each language variant to generate missing AC3 5.1 and AAC stereo. Qualifying untagged main tracks are accepted as existing compatibility audio without claiming verified derivation. Tagged tracks must match the selected source; stereo-only sources are never upmixed to 5.1.

The `MEDIA_OPTIMIZER_AUDIO_SOURCE` stream tag links compatibility tracks to their selected main source. It carries the source's channel count, quality rank, and a hashed identity, not its filename or connection credentials. Matching generated tracks are reused on later runs even when the original 7.1 track was removed. Existing untagged AC3 5.1 and AAC stereo main tracks are also reused for their language variant; missing tags alone never require re-encoding or a remux. If another change requires FFmpeg processing, accepted untagged tracks receive the selected source association without an audio encode. This association records acceptance, not proof that legacy audio was derived from that source or loudness-normalized. Existing mismatched or malformed source tags do not qualify for this fallback.
...
The mode supports native-resolution 4K MKV video with HDR10+, Dolby Vision Profile 7 MEL converted to Profile 8.1, and existing HDR10-compatible Dolby Vision Profile 8.1. Version `0.1.12` also supports exact square-pixel Dolby Vision 3840x2160-to-1920x1080 downscaling under `Normalize to 1080p` or `Downscale 4K to 1080p`. It halves letterbox offsets without cropping and restores the adjusted metadata. Native 1920x1080 Dolby Vision can be compressed when above the selected bitrate ceiling; compliant output does not trigger another encode. Odd offsets that cannot be halved exactly fail before encoding. Cropping, resized HDR10+, FEL/Profile 5, other Dolby Vision compatibility IDs, and combined Dolby Vision + HDR10+ remain unsupported. Out-of-scope configurations stay in copy mode. A full-file check rejects unsupported dynamic metadata before encoding; restoration failures fail the job without publishing the cache output. Use a separate source copy for initial playback validation; metadata checks are not a guarantee of device compatibility or professional regrading.

Version `0.1.13` cross-checks static mastering/light metadata against source MKV headers using a short raw-HEVC sample before encoding, then checks sampled encoded/restored raw frames and final MKV headers against that baseline. Conflicting source values or encoder RGB rotation fail safely; values are not silently guessed or rotated. Static metadata may be carried only in MKV headers when absent from sampled bitstream SEI. Sampling is not a full-file static-metadata audit. This validation does not repair previously affected media or replace FFmpeg; see [HDR tooling](../../docs/hdr_tooling.md) for the known Jellyfin encoder issue.
